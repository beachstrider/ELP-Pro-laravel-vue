...
const STORED_SETTINGS = storedSettings => {
    ...
    Object.keys(storedSettings).forEach(key => {
        const item = store.get(`alcolmLps.settings.${key}`)
...
export default {
    ...
    mutations: {
        CHANGE_SETTING(state, payload) {
            window.localStorage.setItem(`alcolmLps.settings.${payload.setting}`, payload.value)
        ...
        SET_PRIMARY_COLOR(state, payload) {
            ...
            store.set('alcolmLps.settings.primaryColor', color)
        },
        SET_THEME(state, payload) {
            ...
            store.set('alcolmLps.settings.theme', nextTheme)
        },
    },
    actions: {},
    getters: {
