<template>
	<fragment>
		<a :title="$t('button.title.addItem')"
		   v-if="$global.hasPermission('brandsstore') && allowCreate > 0"
		   v-b-tooltip.hover
		...
		<a :title="$t('button.title.editItem')"
		   v-if="$global.hasPermission('brandsupdate') && hasId && allowUpdate > 0"
		   @click="handleEditBrandClick"
		   v-b-tooltip.hover>
    ...
    export default {
	    ...
	    data() {
	        return {
		        ...
		        visibility: false,
                editLoading: false,
                operationTitle: this.$t('title.addBrand')
	        }
	    },
		methods: {
            async handleSubmit() {
                ...
                try {
                    ...
                    if (this.formFields.id) {
	                    this.handleAfterBrandUpdate(this.formFields, this.formFields.id);
                    } else {
                        this.handleAfterBrandCreate(response.data);
                    }
            ...
            async handleEditClick(id) {
                try {
                    ...
                    this.operationTitle = this.$t('title.addBrand')
                    const {data} = response
            ...
            handleAfterBrandCreate(inputs) {
                if (this.afterCreate) {
                    this.afterCreate(inputs)
                }
            },
            handleAfterBrandUpdate(inputs) {
                if (this.afterUpdate) {
