	...
	<div v-if="show">
		<div class="card">
					...
					<div class="row">
						<div class="col-8">
							<h5 class="mt-3 ml-0 mr-3 mb-2">
								<strong>
									<template v-if="operation === null">{{$t('title.logisticTypes')}}</template>
									...
									<b-input-group class="mt-3">
										...
										<b-input-group-append>
											<b-button @click="setOperation('add')" variant="info" size="sm"
											          ...
											          v-if="$global.hasPermission('logistictypesstore')" v-b-tooltip.hover>
											...
											<b-button size="sm" :title="$t('button.title.filterRecords')"
											          ...
											          v-if="$global.hasPermission('logistictypesview')">
			...
			<div class="card-body">
                    ...
                    <b-table hover responsive
                        ...
                        <template v-slot:cell(action)="record">
                            <a @click="setOperation('edit', record.item.id)"
                               :title="$t('button.title.editItem')" v-if="$global.hasPermission('logistictypesupdate')"
                               v-b-tooltip.hover>
                                <i class="fe fe-edit"></i>
                            </a>
                            <a-popconfirm title="Are you sure？" @confirm="handleDeleteClick(record.item.id)"
                                          v-if="$global.hasPermission('logistictypesdestroy')">
                ...
                </div><!-- /.logistictypes-table -->
                <div class="logistictypes-operation">
                    <a-drawer
                        placement="right"
                        :width="'360px'"
...
<script>
    ...
    const COLUMN_DEFINATION = (self) => [
        ...
        (self.$global.hasAnyPermission(['logistictypesupdate', 'logistictypesdestroy'])
        ? {
    ...
    export default {
        ...
        data() {
            return {
                operationTitle: 'title.logisticTypes',
                formFields: {...FORM_STATE},
                filters: {...FILTER_STATE},
                listUrl: 'logistic/types',
        ...
        methods: {
            setOperation(operation = 'add', operationToken = null) {
                this.operationTitle = (operation === 'add' ? 'title.addLogisticType' : 'title.editLogisticType')
                this.$router.replace({
                    query: Object.assign({},
                        this.$route.query,
                        {
            ...
            async handleSubmit() {
                ...
                try {
                    const response = await request({
                        url: this.formFields.id ? 'logistic/types/update' : 'logistic/types/create',
                        method: 'post',
            ...
            hasListAccess() {
                return this.$global.hasPermission('logistictypesview')
            },
            getExtraParams() {
                return {
                    filters: {
