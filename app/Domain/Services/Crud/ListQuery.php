...
trait ListQuery
{
        ...
        if ($searchValue = ArrayHelper::get($input, 'search')) {
            $query->where(function (Builder $query) use ($searchValue) {
                foreach ($this->searchColumns as $column) {
                    if (is_array($column)) {
                        $query->orWhereHas($column['relationship'], function ($qry) use ($searchValue, $column) {
                            $qry->where($column['column'], 'like', '%' . $searchValue . '%');
                        });
                    } else {
    ...
    protected function resultQuery(Builder $query, $input = [])
    {
        if (ArrayHelper::has($input, 'page')) {
            $pageSize = (ArrayHelper::get($input, 'perPage')) ? ArrayHelper::get($input, 'perPage') : self::$perPage;
